const dictionary = {
  en: {
    ...
    factsEyebrow: "Reference",
    factsTitle: "Fact Sheet",
    developer: "Developer",
    releaseDate: "Release Date",
    releaseValue: "Q3 2026",
    demoReleaseDate: "Demo Release Date",
    demoReleaseValue: "June 3rd 2026 - Steam Next Fest",
    demoAnnouncement: "DEMO will be available June 3rd 2026 and will participate in June Steam Next Fest.",
    genre: "Genre",
    ...
    screenshotAlt3: "Ling House screenshot 3",
  ...
  zh: {
    ...
    demoReleaseDate: "试玩版发行日期",
    demoReleaseValue: "2026 年 6 月 3 日 - Steam Next Fest",
    demoAnnouncement: "本作的试玩版将于 2026 年 6 月 3 日 发布并参与6月新品节。",
    genre: "类型",
    genreValue: "心理恐怖",
    platform: "平台",
    contact: "联系邮箱",
  ...
  ja: {
    ...
    demoReleaseDate: "デモ公開日",
    demoReleaseValue: "2026年6月3日 - Steam Next Fest",
    demoAnnouncement: "デモは2026年6月3日に公開され、6月のSteam Next Festに参加します。",
    genre: "ジャンル",
    ...
    screenshotAlt3: "『灵宅：回音缭绕』スクリーンショット 3",
...
const trailerEmbeds = {
  ...
  bilibili: {
    src: "https://player.bilibili.com/player.html?bvid=BV_PLACEHOLDER&page=1&autoplay=0",
    title: "Ling House: Echoes Within trailer on Bilibili"
  }
};
...
function setCommunityContact(lang, strings) {
  communityValue.replaceChildren();

  if (lang === "zh") {
    communityLabel.textContent = strings.qqGroup;
    communityValue.textContent = QQ_GROUP_NUMBER;
    return;
  }
  ...
  link.target = "_blank";
  link.rel = "noopener";
  link.textContent = strings.discordButton;

  communityLabel.textContent = strings.discord;
  communityValue.appendChild(link);
}
...
function applyLanguage(lang) {
  ...
  document.querySelectorAll("[data-i18n-attr]").forEach((node) => {
    node.dataset.i18nAttr.split(",").forEach((pair) => {
      ...
      if (attr && key && typeof strings[key] === "string") {
        node.setAttribute(attr.trim(), strings[key]);
      }
    });
  });
  ...
  setCommunityContact(lang, strings);
  setTrailer(lang);
}

function openLightbox(card) {
  const image = card.querySelector("img");
  const fullSrc = card.dataset.full || image.src;
